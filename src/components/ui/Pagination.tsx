...
export const Pagination: React.FC<PaginationProps> = ({
    ...
    return (
        <div className={cn('flex w-full items-center', hasLeftContent ? 'justify-between' : 'justify-center', className)}>
            {hasLeftContent && (
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 font-medium">
                <button
                    ...
                    className="rounded-lg border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer transition-all hover:shadow-sm h-9 w-9 flex justify-center items-center text-gray-600 dark:text-gray-400 shrink-0"
                    title="First Page"
                >
                ...
                <button
                    ...
                    disabled={currentPage <= 1}
                    className="flex items-center justify-center gap-1 rounded-lg border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer transition-all hover:shadow-sm group h-9 w-9 md:w-16 text-gray-600 dark:text-gray-400 shrink-0"
                    title="Previous Page"
                >
                ...
                <div className="flex items-center gap-1 mx-0.5">
                    {visiblePages.map((page, index) => (
                        <React.Fragment key={index}>
                            {page === '...' ? (
                                <span className="px-1 text-gray-400 dark:text-gray-600 font-medium italic text-xs">...</span>
                            ) : (
                                <button
                                    onClick={() => onPageChange(page as number)}
                                    className={`min-w-[36px] h-9 rounded-lg text-xs font-bold transition-all duration-200 cursor-pointer ${currentPage === page
                ...
                <button
                    ...
                    disabled={currentPage >= effectiveTotalPages}
                    className="rounded-lg border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 h-9 w-9 flex justify-center items-center disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer transition-all hover:shadow-sm text-gray-600 dark:text-gray-400 shrink-0"
                    title="Last Page"
                >
