...
export default function ProductDetailView({ product }: ProductDetailViewProps) {
  ...
  >(() => {
    ...
    if (product.swatches) {
      product.swatches.forEach((sw) => {
        if (sw.options && sw.options.length > 0) {
          initial[sw.category] = sw.options[0].name;
        }
      });
    }
    return initial;
  ...
  // Curated premium options swatches with HEX and descriptions
  const tabletopSwatches: ColorSwatch[] = [
    { name: "Premium Oak", hex: "#d8b589", desc: "Warm natural oak woodgrain" },
    {
      name: "Bavarian Beech",
      hex: "#e5caab",
      desc: "Light warm clean beechwood",
    },
    {
      name: "Frosty White",
      hex: "#f8f9fa",
      border: true,
      desc: "Clean glossy corporate white",
    },
    {
      name: "Slate Grey",
      hex: "#6c757d",
      desc: "Modern architectural solid grey",
    },
  ];

  const frameSwatches: ColorSwatch[] = [
    {
      name: "Matte Black",
      hex: "#1e1e1e",
      desc: "Deep textured protective coating",
    },
    {
      name: "Pearl White",
      hex: "#ffffff",
      border: true,
      desc: "Sleek modern white metal coating",
    },
    {
      name: "Satin Silver",
      hex: "#ced4da",
      desc: "Clean anodized brushed metal style",
    },
  ];

  const panelSwatches: ColorSwatch[] = [
    {
      name: "Cobalt Blue Fabric",
      hex: "#0f52ba",
      desc: "Soft acoustic felt board",
    },
    {
      name: "Charcoal Felt",
      hex: "#495057",
      desc: "Sound-dampening grey acoustic felt",
    },
    {
      name: "Frosted Glass",
      hex: "#e2e8f0",
      border: true,
      desc: "Semi-transparent modern privacy screen",
    },
  ];

  const chairMeshSwatches: ColorSwatch[] = [
    {
      name: "Charcoal Aero Mesh",
      hex: "#2d3748",
      desc: "Advanced tensile cooling grid mesh",
    },
    {
      name: "Steel Blue Cushion",
      hex: "#1a365d",
      desc: "High-density foam with micro-weave fabric",
    },
    {
      name: "Crimson Red Mesh",
      hex: "#9b2c2c",
      desc: "Vibrant high-contrast nylon mesh",
    },
    {
      name: "Executive Black Leatherette",
      hex: "#1a1a1a",
      desc: "Premium textured faux-leather",
    },
  ];

  const chairBaseSwatches: BaseSwatch[] = [
    {
      name: "Reinforced Nylon Base",
      desc: "Extremely durable structural matte black base",
    },
    {
      name: "Polished Aluminium Base",
      desc: "Stunning mirror-finish metallic chrome base",
    },
  ];

  const [activeTabletop, setActiveTabletop] = useState(
    tabletopSwatches[0].name,
  );
  const [activeFrame, setActiveFrame] = useState(frameSwatches[0].name);
  const [activePanel, setActivePanel] = useState(panelSwatches[0].name);
  const [activeChairMesh, setActiveChairMesh] = useState(
    chairMeshSwatches[0].name,
  );
  const [activeChairBase, setActiveChairBase] = useState(
    chairBaseSwatches[0].name,
  );
    ...
    return (
      <div className="space-y-10">
        <div className="space-y-3">
          ...
          <p className="text-xs text-neutral-400 font-normal leading-relaxed max-w-2xl">
            Match your interior branding perfectly. Choose from selected premium
            material palettes constructed in our factory and backed by
            zero-defect durability.
          </p>
        </div>

        {product.swatches && product.swatches.length > 0 ? (
          <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
            {product.swatches.map((sw, index) => {
              const currentVal =
                selectedSwatches[sw.category] || sw.options[0]?.name || "";
              return (
                <div
                  key={index}
                  className="space-y-5 bg-neutral-50/20 p-5 rounded-xl border border-neutral-100/70"
                >
                  <div className="space-y-0.5">
                    <span className="text-[8px] font-semibold text-neutral-400 uppercase tracking-widest">
                      Section {(index + 1).toFixed(1)}
                    </span>
                    <h4 className="text-xs font-bold text-secondary/60 uppercase tracking-widest">
                      {sw.category}
                    </h4>
                  </div>
                      ...
                      <div
                        ...
                        onClick={() =>
                          setSelectedSwatches((prev) => ({
                            ...prev,
                            [sw.category]: opt.name,
                          }))
                        }
                        className={cn(
                          "flex items-center gap-3.5 p-3.5 rounded-xl border cursor-pointer transition-all duration-300",
                          currentVal === opt.name
                            ? "bg-white border-primary shadow-soft scale-[1.02]"
                            : "bg-white/40 border-neutral-50 hover:border-neutral-200 hover:bg-white",
                        )}
                      >
                        <div
                          className="w-8.5 h-8.5 rounded-lg shrink-0 border border-neutral-200/50 shadow-inner relative flex items-center justify-center"
                        ...
                        >
                          {currentVal === opt.name && (
                            <Check
                              ...
                              className={cn(
                                "text-white",
                                opt.border && "text-secondary",
                              )}
                            />
                          )}
                        </div>
                        <div>
                          <h5 className="text-[11px] font-semibold text-secondary">
                            {opt.name}
                          </h5>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {/* Mesh customizer */}
            <div className="space-y-6 bg-neutral-50/30 p-6 rounded-xl border border-neutral-100">
              <div className="space-y-1">
                <span className="text-[9px] font-semibold text-neutral-400 uppercase tracking-widest">
                  Section 1.0
                </span>
                <h4 className="text-sm font-bold text-secondary uppercase tracking-tight">
                  Mesh & Upholstery
                </h4>
              </div>

              <div className="space-y-4">
                {chairMeshSwatches.map((u, i) => (
                  <div
                    key={i}
                    onClick={() => setActiveChairMesh(u.name)}
                    className={cn(
                      "flex items-center gap-4 p-4 rounded-xl border cursor-pointer shadow-sm transition-all duration-300",
                      activeChairMesh === u.name
                        ? "bg-white border-primary shadow-[0_10px_25px_-10px_rgba(237,28,39,0.15)] scale-102"
                        : "bg-white/40 border-neutral-100 hover:border-neutral-300 hover:bg-white",
                    )}
                  >
                    <div
                      className="w-10 h-10 rounded-xl shrink-0 border border-neutral-200/50 shadow-inner relative flex items-center justify-center"
            ...
            <div className="space-y-6 bg-neutral-50/30 p-6 rounded-xl border border-neutral-100">
              ...
              <div className="space-y-4">
                {chairBaseSwatches.map((b, i) => (
                  <div
                    key={i}
                    onClick={() => setActiveChairBase(b.name)}
                    className={cn(
                      "p-5 rounded-xl border cursor-pointer shadow-sm transition-all duration-300",
                      activeChairBase === b.name
                        ? "bg-white border-primary shadow-[0_10px_25px_-10px_rgba(237,28,39,0.15)] scale-102"
                        : "bg-white/40 border-neutral-100 hover:border-neutral-300 hover:bg-white",
                    )}
                  >
                    ...
                    <p className="text-[10px] text-neutral-400 font-normal mt-1 leading-normal">
                      {b.desc}
                    </p>
                  </div>
                ))}
          ...
          <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
            {/* Tabletop finishes column */}
            <div className="space-y-5 bg-neutral-50/20 p-5 rounded-xl border border-neutral-100/70">
              <div className="space-y-0.5">
                <span className="text-[8px] font-semibold text-neutral-400 uppercase tracking-widest">
                  Element A
                </span>
              ...
              <div className="space-y-3">
                {tabletopSwatches.map((t, i) => (
                  <div
                    key={i}
                    onClick={() => setActiveTabletop(t.name)}
                    className={cn(
                      "flex items-center gap-3.5 p-3.5 rounded-xl border cursor-pointer transition-all duration-300",
                      activeTabletop === t.name
                        ? "bg-white border-primary shadow-soft scale-[1.02]"
                        : "bg-white/40 border-neutral-50 hover:border-neutral-200 hover:bg-white",
                    )}
                  >
            ...
            <div className="space-y-5 bg-neutral-50/20 p-5 rounded-xl border border-neutral-100/70">
              ...
              <div className="space-y-3">
                {frameSwatches.map((m, i) => (
                  <div
                    key={i}
                    onClick={() => setActiveFrame(m.name)}
                    className={cn(
                      "flex items-center gap-3.5 p-3.5 rounded-xl border cursor-pointer transition-all duration-300",
                      activeFrame === m.name
                        ? "bg-white border-primary shadow-soft scale-[1.02]"
            ...
            {/* Acoustic Dividers / Side board panel options */}
            <div className="space-y-5 bg-neutral-50/20 p-5 rounded-xl border border-neutral-100/70">
              <div className="space-y-0.5">
                <span className="text-[8px] font-semibold text-neutral-400 uppercase tracking-widest">
                  Element C
                ...
                <h4 className="text-xs font-bold text-secondary/60 uppercase tracking-widest">
                  {isWorkstation
                    ? "Partition screens"
                    : "Cabinet lining panels"}
                </h4>
              </div>

              <div className="space-y-3">
                {panelSwatches.map((d, i) => (
                  <div
                    key={i}
                    onClick={() => setActivePanel(d.name)}
                    className={cn(
                      "flex items-center gap-3.5 p-3.5 rounded-xl border cursor-pointer transition-all duration-300",
                      activePanel === d.name
                        ? "bg-white border-primary shadow-soft scale-[1.02]"
