...
import { Plus, Trash2, Upload, ArrowLeft, Save, CheckCircle2 } from "lucide-react";
import Image from "next/image";
import { Card, useAppToast, AdminPageHeader, RHFControl } from "@/components/ui";
...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (watchedStatus === "Inactive") setValue("stock", 0);
  ...
  if (notFound) {
    return (
      <div className="space-y-6">
        <AdminPageHeader title="Edit Product" breadcrumbs={[{ label: "Admin", href: AppRoutes.Admin.Dashboard }, { label: "Products", href: AppRoutes.Admin.Products }, { label: "Not Found" }]} />
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          <p className="text-sm font-semibold text-gray-500">Product with ID <code className="px-2 py-0.5 bg-gray-100 rounded text-xs font-mono">{id}</code> was not found.</p>
          <button onClick={() => router.push(AppRoutes.Admin.Products)} className="inline-flex items-center gap-2 px-4 py-2 bg-[#ed1c27] text-white text-xs font-bold uppercase tracking-wider rounded-xl cursor-pointer">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            <button type="submit" disabled={isSubmitting || savedSuccess}
              className={`inline-flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold uppercase tracking-wider text-xs text-white shadow-lg transition-all duration-300 cursor-pointer ${
                savedSuccess ? "bg-emerald-500 shadow-emerald-500/20" : "bg-[#ed1c27] hover:bg-[#c5141e] shadow-[#ed1c27]/20 hover:-translate-y-0.5"
              } disabled:opacity-60`}>
              {savedSuccess ? <><CheckCircle2 size={14} /> Saved!</> : <><Save size={14} /> Save Changes</>}
            </button>
          </div>
          ...
          <div className="grid lg:grid-cols-[1fr_320px] gap-6 items-start">
                ...
                {TABS.map((tab) => (
                  <button key={tab.key} type="button" onClick={() => setActiveTab(tab.key)}
                    className={`flex-shrink-0 py-3.5 px-4 text-[10px] font-bold uppercase tracking-wider border-b-2 whitespace-nowrap transition-all duration-300 ${
                      activeTab === tab.key ? "border-[#ed1c27] text-[#ed1c27]" : "border-transparent text-gray-400 hover:text-gray-600"
                    }`}>
                    {tab.label}
                  </button>
                ))}
              ...
              <Card.Body>
                ...
                <div className={activeTab === "general" ? "space-y-5" : "hidden"}>
                  ...
                  <RHFControl control="select" name="status" label="Status *"
                    options={[{ label: "Active", value: "Active" }, { label: "Inactive", value: "Inactive" }]} className="rounded-xl" />
                  <RHFControl control="textarea" name="description" label="Description *" placeholder="Provide a detailed description..." className="rounded-xl" />
                </div>

                {/* MEDIA */}
                <div className={activeTab === "media" ? "space-y-5" : "hidden"}>
                  <div className="space-y-2">
                    <label className="text-[10px] font-semibold uppercase tracking-widest text-secondary/60">Product Images (at least 1 required) *</label>
                    <Controller name="images" control={control}
                      render={({ field, fieldState: { error } }) => {
                        const handleFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
                          const files = e.target.files;
                          if (!files?.length) return;
                          const promises = Array.from(files).map((f) => new Promise<string>((res) => { const r = new FileReader(); r.onloadend = () => res(r.result as string); r.readAsDataURL(f); }));
                          Promise.all(promises).then((urls) => field.onChange([...(field.value || []), ...urls]));
                        };
                        const removeAt = (i: number) => { const arr = [...(field.value || [])]; arr.splice(i, 1); field.onChange(arr); };
                        return (
                              ...
                              <div className="grid grid-cols-3 gap-4">
                                {field.value.map((url: string, i: number) => (
                                  <div key={i} className="relative aspect-video rounded-xl overflow-hidden border border-gray-100 group/img">
                                    ...
                                    <button type="button" onClick={() => removeAt(i)} className="absolute top-2 right-2 p-1 bg-red-500 text-white rounded-lg cursor-pointer opacity-0 group-hover/img:opacity-100 z-10"><Trash2 size={12} /></button>
                                    {i === 0 && <span className="absolute bottom-2 left-2 px-2 py-0.5 bg-black/60 text-white text-[9px] font-bold uppercase tracking-wider rounded-md">Primary</span>}
                                  </div>
                                ))}
                              </div>
                            )}
                          ...
                          <div className="border-2 border-dashed border-gray-200 hover:border-[#ed1c27]/50 rounded-xl p-5 flex flex-col items-center justify-center gap-2 bg-gray-50/50 relative group cursor-pointer transition-all duration-300 min-h-[120px]">
                            {field.value ? (
                              <div className="relative w-full h-32 rounded-lg overflow-hidden">
                                ...
                                <button type="button" onClick={(e) => { e.stopPropagation(); field.onChange(""); }} className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-lg cursor-pointer z-10"><Trash2 size={10} /></button>
                              </div>
                            ) : (
                              <><Upload className="w-6 h-6 text-gray-400 group-hover:text-[#ed1c27] transition-colors" /><span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Upload Blueprint Image</span></>
                            )}
                ...
                <div className={activeTab === "specs" ? "space-y-8" : "hidden"}>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                      ...
                      <button type="button" onClick={() => appendSpec({ label: "", value: "" })} className="inline-flex items-center gap-1 bg-blue-50 text-blue-600 hover:bg-blue-100 text-[10px] font-bold uppercase tracking-wider rounded-lg px-3 py-1.5 cursor-pointer"><Plus size={10} /> Add Spec</button>
                    </div>
                    {specFields.length === 0 ? <p className="text-[10px] text-gray-400 py-2">No specifications yet.</p> : (
                      <div className="space-y-3">
                        {specFields.map((f, i) => (
                          <div key={f.id} className="flex gap-3 items-end">
                            ...
                            <button type="button" onClick={() => removeSpec(i)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg cursor-pointer mb-1"><Trash2 size={14} /></button>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                      ...
                      <button type="button" onClick={() => appendFeature({ title: "", desc: "" })} className="inline-flex items-center gap-1 bg-blue-50 text-blue-600 hover:bg-blue-100 text-[10px] font-bold uppercase tracking-wider rounded-lg px-3 py-1.5 cursor-pointer"><Plus size={10} /> Add Feature</button>
                    </div>
                    {featureFields.length === 0 ? <p className="text-[10px] text-gray-400 py-2">No features yet.</p> : (
                      <div className="space-y-3">
                        {featureFields.map((f, i) => (
                          <div key={f.id} className="flex gap-3 items-end">
                            ...
                            <div className="flex-[2]"><RHFControl control="input" name={`features.${i}.desc`} label={i === 0 ? "Description *" : ""} placeholder="e.g. Contour backrest with adjustments" className="rounded-xl" /></div>
                            <button type="button" onClick={() => removeFeature(i)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg cursor-pointer mb-1"><Trash2 size={14} /></button>
                          </div>
                        ))}
                      </div>
                    )}
                ...
                <div className={activeTab === "advanced" ? "space-y-8" : "hidden"}>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                      ...
                      <button type="button" onClick={() => appendDim({ name: "", range: "", coord: "" })} className="inline-flex items-center gap-1 bg-blue-50 text-blue-600 hover:bg-blue-100 text-[10px] font-bold uppercase tracking-wider rounded-lg px-3 py-1.5 cursor-pointer"><Plus size={10} /> Add Dimension</button>
                    </div>
                    {dimFields.length === 0 ? <p className="text-[10px] text-gray-400 py-2">No dimensions yet.</p> : (
                      <div className="space-y-3">
                        {dimFields.map((f, i) => (
                          <div key={f.id} className="flex gap-2 items-end">
                            ...
                            <button type="button" onClick={() => removeDim(i)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg cursor-pointer mb-1"><Trash2 size={14} /></button>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                      ...
                      <button type="button" onClick={() => appendRes({ id: `res-${Date.now()}`, title: "", desc: "", format: "PDF", size: "2.4 MB" })} className="inline-flex items-center gap-1 bg-blue-50 text-blue-600 hover:bg-blue-100 text-[10px] font-bold uppercase tracking-wider rounded-lg px-3 py-1.5 cursor-pointer"><Plus size={10} /> Add Resource</button>
                    </div>
                    {resFields.length === 0 ? <p className="text-[10px] text-gray-400 py-2">No resources yet.</p> : (
                      <div className="space-y-3">
                        {resFields.map((f, i) => (
                ...
                <div className={activeTab === "variants" ? "space-y-8" : "hidden"}>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                      ...
                      <button type="button" onClick={() => appendVariant({ label: "", options: [] })} className="inline-flex items-center gap-1 bg-violet-50 text-violet-600 hover:bg-violet-100 text-[10px] font-bold uppercase tracking-wider rounded-lg px-3 py-1.5 cursor-pointer shrink-0"><Plus size={10} /> Add Variant</button>
                    </div>
                    {variantFields.length === 0 ? <p className="text-[10px] text-gray-400 py-2">No variants added.</p> : (
                      <div className="space-y-3">
                        {variantFields.map((f, i) => (
                          <div key={f.id} className="border border-gray-100 rounded-xl p-4 bg-gray-50/40 space-y-3 relative">
                            ...
                            <RHFControl control="input" name={`variants.${i}.label`} label="Variant Label *" placeholder="e.g. Size, Frame Type" className="rounded-xl" />
                              ...
                              render={({ field: optField }) => {
                                ...
                                return (
                                  <div className="space-y-1.5">
                                    <label className="text-[10px] font-semibold uppercase tracking-widest text-secondary/60">Options <span className="text-gray-400 normal-case tracking-normal font-normal">(comma-separated)</span></label>
                                    ...
                                    {Array.isArray(optField.value) && (optField.value as string[]).length > 0 && (
                                      <div className="flex flex-wrap gap-1.5 mt-1.5">
                                        {(optField.value as string[]).map((opt, oi) => <span key={oi} className="px-2 py-0.5 bg-violet-50 text-violet-700 text-[10px] font-semibold rounded-md border border-violet-100">{opt}</span>)}
                                      </div>
                                    )}
                                  </div>
                                );
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                      ...
                      <button type="button" onClick={() => appendSwatch({ category: "", options: [] })} className="inline-flex items-center gap-1 bg-amber-50 text-amber-600 hover:bg-amber-100 text-[10px] font-bold uppercase tracking-wider rounded-lg px-3 py-1.5 cursor-pointer shrink-0"><Plus size={10} /> Add Swatch Category</button>
                    </div>
                    {swatchFields.length === 0 ? <p className="text-[10px] text-gray-400 py-2">No swatch categories.</p> : (
                      <div className="space-y-4">
            ...
            <div className="space-y-4 lg:sticky lg:top-20">
              <Card>
                <Card.Header><h3 className="text-xs font-bold uppercase tracking-widest text-gray-700">Form Sections</h3></Card.Header>
                <Card.Body>
                  <div className="space-y-1">
                    {TABS.map((tab) => (
                      <button key={tab.key} type="button" onClick={() => setActiveTab(tab.key)}
                        className={`w-full text-left px-3 py-2 rounded-lg text-xs font-semibold transition-all duration-200 ${activeTab === tab.key ? "bg-[#ed1c27]/8 text-[#ed1c27] font-bold" : "text-gray-500 hover:bg-gray-50 hover:text-gray-800"}`}>
                        {tab.label}
                      </button>
                    ))}
                  </div>
          ...
          <div className="sticky bottom-0 z-30 bg-white/95 backdrop-blur border-t border-gray-100 -mx-6 px-6 py-3 flex items-center justify-between gap-4 shadow-[0_-4px_20px_-8px_rgba(0,0,0,0.08)]">
            <p className="text-[10px] text-gray-400 font-medium">Changes are saved to session storage and reflected instantly on the storefront.</p>
            <button type="submit" disabled={isSubmitting || savedSuccess}
              className={`inline-flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold uppercase tracking-wider text-xs text-white transition-all duration-300 cursor-pointer ${savedSuccess ? "bg-emerald-500" : "bg-[#ed1c27] hover:bg-[#c5141e] hover:-translate-y-0.5"} disabled:opacity-60`}>
              {savedSuccess ? <><CheckCircle2 size={14} /> Saved!</> : <><Save size={14} /> Save Changes</>}
            </button>
          </div>
        </form>
